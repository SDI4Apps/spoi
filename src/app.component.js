...
export const AppComponent = {
  ...
  controller: function (
    ...
    $scope.HsCore = HsCore;
    $scope.queryData = HsQueryBaseService.data;

    $scope.$on('scope_loaded', (event, args, arg2) => {
      HsLayoutService.setMainPanel('layermanager');
      if (args == 'Sidebar') {
        const el = angular.element(
          '<div hs.trip_planner.directive hs.draggable ng-controller="hs.trip_planner.controller" ng-if="HsCore.exists(\'hs.trip_planner.controller\')" ng-show="HsCore.panelVisible(\'trip_planner\', this)"></div>'
        );
    ...
    $scope.addToTrip = function () {
      HsTripPlannerService.addWaypoint($scope.lon_lat[0], $scope.lon_lat[1]);
    ...
    /* Not really used anymore
        $scope.$on('feature_crossfilter_filtered', function(event, data) {
            var lyr = HsMapService.findLayerByTitle('Specific points of interest');
    ...
    $scope.showDeveloperInfo = function () {
      ...
      const el = angular.element('<div hs.advanced_infopanel_directive></div>');
      $('#hs-dialog-area').append(el);
      $compile(el)($scope);
    };
